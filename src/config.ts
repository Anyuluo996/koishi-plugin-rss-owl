...
export const Config: Schema<ConfigType> = Schema.object({
  basic: Schema.object({
    ...
    resendUpdatedContent: Schema.union(['disable','latest','all']).description('当内容更新时再次发送').default('disable'),
    resendUpdataContent: Schema.union(['disable','latest','all']).description('（已弃用，请使用 resendUpdatedContent）').hidden().deprecated(),
    ...
    mergeVideo: Schema.boolean().default(false).description('以合并消息发送视频'),
    margeVideo: Schema.boolean().description('（已弃用，请使用 mergeVideo）').hidden().deprecated(),
